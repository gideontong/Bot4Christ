...
class Bible {
    ...
    static buildEmbed(bot, query = '', daily = false) {
        ...
        if (query && !daily) {
            ...
            if (references.length == 1) {
                ...
                let text = bible[references[0][0]][references[0][1]][references[0][2]];
                ...
                for (let i = 0; i < verseLength; i++) {
                    ...
                    let text = bible[references[i][0]][references[i][1]][references[i][2]];
                    if (text) {
                        count++;
                    } else {
                        continue;
        ...
        for (let range of referenceRanges) {
            if (range.includes('-')) {
                ...
                try {
                    ...
                    log.error(`Trying to add references got a ${err}`);
                }
            } else {
    ...
    static addReferenceRange(references, start, end) {
        if (references.length > MAX_VERSES || !start[0] || !end[0]) {
            return;
        }
